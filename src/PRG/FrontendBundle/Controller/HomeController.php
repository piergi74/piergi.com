...
class HomeController extends Controller
{
  ...
  public function homeAction(Request $request, $_locale) {
    ...
    $repository = $this->get('doctrine_mongodb')->getManager()->getRepository('FrontendBundle:User');
    $user = $repository->findOneByLinkedinId('kZZE2X_xU3');   
    //dump($user);
  ...
  public function updateAction(Request $request)
  {
    ...
    //$generated_state = $li->getState();
    //print_r('$generated_state: '.$generated_state);
    //die;
    ...
    if($request->get('error')) {
      ...
      die;
    } elseif($request->get('code') && empty($sessionAccessToken)) {
      // User authorized your application
      if ($request->get('state') == $request->get('state') &&
              empty($session->get('linkedin_tokens/expires_at')) &&
              (time() > $session->get('linkedin_tokens/expires_at')) &&
              empty($session->get('linkedin_tokens/access_token'))
              ) {

          // Get token so you can make API calls
          $token = $li->getAccessToken($request->get('code'));
          //$token = $li->getAccessToken($_REQUEST['code']);
          $token_expires = $li->getAccessTokenExpiration();  //return a json object
        ...
        if((empty($session->get('linkedin_tokens/expires_at'))) || (time() > $session->get('linkedin_tokens/expires_at'))) {
            // Token has expired, clear the state
            //$_SESSION = array();
            $session->remove('linkedin_tokens');
        }
        if (empty($session->get('linkedin_tokens/access_token'))) {
    ...
    */
    ...
    ->getManager()
    ->getRepository('FrontendBundle:User');
    ...
    */
    ...
    if($user) {
      if($updatedInLinkedin < $updatedInMongo) {
        ...
        $user->setEducations($info['educations']);
        $user->setLanguages($info['languages']);
        $user->setPositions($info['positions']);
        $user->setSkills($info['skills']);
   ...
   */    
  public function updateFromLinkedinAction($info)
  {
